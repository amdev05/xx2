...
export default function HeroSlide({ data, isActive }) {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className="absolute bottom-20 left-0 w-full flex flex-col items-center md:items-start md:left-10 mask-x-to-yellow-100 md:w-fit">
        <img src={data.logo} alt="" className="max-w-50 max-h-20" />
        ...
        <Button classname={"mt-5 w-65 md:w-fit"} onClick={() => navigate(`/cinema/${data.id}`)}>
          Beli Tiket Sekarang
        </Button>
      </div>
    </div>
